...
function BodyEditor() {
    ...
    const inputURL = useAppSelector(state => state.selected.selectedContent).inputURL
 ...
 const [bodyPost, setBodyPost] = useState( decodeURIComponent(bodyfromhistory) ??  '');
  ...
  const handleBodyRequest = ()=> {
    ...
    if(bodyfromhistory?.length >0 ){
      ...
      null,
                          '',
                          '/personal' +     '/' + historyMethod + '/'+
                        
                          (inputfromhistory ?? '') + '/' + (bodyPost ?? '') + '?' + (headers ?? '')
                        );
    }
    ...
    window.history.pushState(
      ...
      (inputURL ?? '')  +   (bodyRequest.length > 0 ? '/'+ bodyRequest : '') + '?' + (headers ?? '')
    );
