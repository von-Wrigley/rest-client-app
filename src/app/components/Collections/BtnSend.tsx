...
const bodyPost = useAppSelector(state => state.selected.selectedContent).bodyReq
const headers = useAppSelector(state => state.selected.selectedContent).headers


let normalURL = atob(inputState);

const handlerequest = async () => {
  const provewithoutVariables =  variablestorage.map((x)=>normalURL.includes(x.key) )

if(provewithoutVariables){
  const newX =  variablestorage.map((x)=> {
    const newstr = normalURL.replace(new RegExp('\{\{(?:\\s+)?(' + x.key+ ')(?:\\s+)?\}\}'), x.value );
    normalURL= newstr
